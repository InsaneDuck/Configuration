...
complete -c winebuild -l data-only -d 'Build a module that contains only data and resources, and no executable code'
...
complete -c winebuild -l syscall-table -d 'Set the system call table id, between 0 and 3'
...
complete -c winebuild -l without-dlltool -d 'Generate import library without using dlltool'
