# mpv
# Autogenerated from man page /usr/lib/jvm/default/man/man1/mpv.1.gz
...
complete -c mpv -l sub-border-color -d 'See --sub-color.  Color used for the sub font border'
...
complete -c mpv -l sub-shadow-color -d 'See --sub-color.  Color used for sub text shadow. sp NOTE: NDENT 7. 0 NDENT 3'
...
complete -c mpv -l x11-present -d 'Whether or not to use presentation statistics from X11\\(aqs presentation exte…'
...
complete -c mpv -l interpolation-preserve -d 'Preserve the previous frames\\(aq interpolated results even when renderer para…'
...
complete -c mpv -l wayland-disable-vsync -d 'Disable mpv\\(aqs internal vsync for Wayland-based video output (default: no)'
...
complete -c mpv -l tone-mapping-crosstalk -d 'If nonzero, apply an extra crosstalk matrix before tone mapping'
complete -c mpv -l tone-mapping-max-boost -d 'Upper limit for how much the tone mapping algorithm is allowed to boost the a…'
complete -c mpv -l tone-mapping-mode -d 'Controls how the tone mapping function is applied to colors.  NDENT 7. 0'
...
complete -c mpv -l cover-art-whitelist -d 'Whether to load filenames in an internal whitelist, such as cover'
