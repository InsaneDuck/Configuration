# sudo
# Autogenerated from man page /usr/lib/jvm/default/man/man8/sudo.8.gz
...
complete -c sudo -s e -d 'Edit one or more file Ns s instead of running a command'
...
complete -c sudo -s K -d 'Similar to the k option, except that it removes every cached credential for t…'
complete -c sudo -s k -d 'When used without a command, invalidates the user\'s cached credentials for th…'
complete -c sudo -s l -d 'If no command is specified, list the privileges for the invoking user (or the…'
complete -c sudo -s N -d 'Do not update the user\'s cached credentials, even if the user successfully au…'
