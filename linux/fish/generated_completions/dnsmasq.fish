# dnsmasq
# Autogenerated from man page /usr/lib/jvm/default/man/man8/dnsmasq.8.gz
...
complete -c dnsmasq -l fast-dns-retry -d 'Under normal circumstances, dnsmasq relies on DNS clients to do retries; it d…'
...
complete -c dnsmasq -l port-limit -d 'By default, when sending a query via random ports to multiple upstream server…'
...
complete -c dnsmasq -s S -l local -l server -d 'Specify upstream servers directly'
...
complete -c dnsmasq -l no-round-robin -d 'Dnsmasq normally permutes the order of A or AAAA records for the same name on…'
complete -c dnsmasq -l use-stale-cache -d 'When set, if a DNS name exists in the cache, but its time-to-live has expired…'
...
complete -c dnsmasq -l no-ident -d 'Do not respond to class CHAOS and type TXT in domain bind queries'
