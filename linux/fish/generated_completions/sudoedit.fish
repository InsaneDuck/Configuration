# sudoedit
# Autogenerated from man page /usr/lib/jvm/default/man/man8/sudoedit.8.gz
...
complete -c sudoedit -s e -d 'Edit one or more file Ns s instead of running a command'
...
complete -c sudoedit -s K -d 'Similar to the k option, except that it removes every cached credential for t…'
complete -c sudoedit -s k -d 'When used without a command, invalidates the user\'s cached credentials for th…'
complete -c sudoedit -s l -d 'If no command is specified, list the privileges for the invoking user (or the…'
complete -c sudoedit -s N -d 'Do not update the user\'s cached credentials, even if the user successfully au…'
